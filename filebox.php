...
$mail = 'yinluobing2@gmail.com'; //若有恶意登录，会发邮件到这个邮箱，前提是mail()函数可用！
...
if ($_COOKIE['user'] != $user || $_COOKIE['pass'] != md5($pass)) {
    if (htmlspecialchars($_REQUEST['user']) == $user && htmlspecialchars($_REQUEST['pass']) == $pass) {
        ...
        if (htmlspecialchars($_REQUEST['user']) == $user || htmlspecialchars($_REQUEST['pass'])) {
            $er = true;
        }
...
function maintop($title, $showtop = true) {
    ...
    if ($showtop) {
//头部菜单内容
        if ($op == 'up' || $op == 'upload' || $op == 'yupload') {
            $up = "class='curr'";
        }
        if ($op == 'home' || $op == 'edit' || $op == 'ren' || $op == 'unz') {
            $home = "class='curr'";
        }
        if ($op == 'cr' || $op == 'create') {
            $cr = "class='curr'";
        }
        if ($op == 'sqlb' || $op == 'sqlbackup') {
            $sqlb = "class='curr'";
        }
        if ($op == 'ftpa' || $op == 'ftpall') {
            $ftpa = "class='curr'";
        }
...
function home() {
    ...
    while ($stylesheet = readdir($style)) {
        ...
        if ($os !== 'windows'): $qx = "<td>" . substr(sprintf('%o', fileperms($ufolder . $sstylesheet)), -3) . "</td>";
            $xx = '<td></td>';else:$qx = '';
            $xx = '';endif;
...
function edit($fename) {
    ...
    if (file_exists($file)) {
        ...
        if (function_exists('mb_detect_encoding')) {
            $encode = mb_detect_encoding($contents, array('ASCII', 'UTF-8', 'GBK', 'GB2312'));
        } else {
            $encode = 'UTF-8';
        }
        ...
        foreach (mb_list_encodings() as $key => $value) {
            if ($key >= 19):
                ...
                if (array_key_exists($value, $arr)) {
                    $value_text = $arr[$value];
                } else {
                    $value_text = $value;
                }

                if ($encode == $value) {
                    $list .= "<option value='$value' selected>" . $value_text . '</option>';
                } else {
                    $list .= "<option value='$value'>" . $value_text . '</option>';
                }
...
function save($ncontent, $fename, $encode) {
    ...
    if (!$fename == "") {
        ...
        if ($encode !== "UTF-8" && $encode !== "ASCII") {
            $ydata = mb_convert_encoding($ydata, $encode, "UTF-8");
        }
